...
export default class Deparser {
  ...
  deparseNodes(nodes, context) {
    return nodes.map(node => {
      return _.isArray(node) ? this.list(node, ', ', '', context) : this.deparse(node, context);
    });
  }
  ...
  ['A_Expr'](node, context = {}) {
    ...
    switch (node.kind) {
      ...
      case 'AEXPR_SIMILAR':
        ...
        if (node.name[0].String.str === '~') {
          if (node.rexpr.FuncCall.args.length > 1) {
            output.push(format('SIMILAR TO %s ESCAPE %s', this.deparse(node.rexpr.FuncCall.args[0], context), this.deparse(node.rexpr.FuncCall.args[1], context)));
          } else {
            output.push(format('SIMILAR TO %s', this.deparse(node.rexpr.FuncCall.args[0], context)));
          }
        } else {
          if (node.rexpr.FuncCall.args.length > 1) {
            output.push(format('NOT SIMILAR TO %s ESCAPE %s', this.deparse(node.rexpr.FuncCall.args[0], context), this.deparse(node.rexpr.FuncCall.args[1], context)));
          } else {
      ...
      case 'AEXPR_BETWEEN_SYM':
        output.push(this.deparse(node.lexpr, context));
        output.push(format('BETWEEN SYMMETRIC %s AND %s', this.deparse(node.rexpr[0], context), this.deparse(node.rexpr[1], context)));
        return output.join(' ');

      case 'AEXPR_NOT_BETWEEN_SYM':
        output.push(this.deparse(node.lexpr, context));
        output.push(format('NOT BETWEEN SYMMETRIC %s AND %s', this.deparse(node.rexpr[0], context), this.deparse(node.rexpr[1], context)));
        return output.join(' ');
      // case 15:
      // AEXPR_PAREN
  ...
  ['BoolExpr'](node, context = {}) {
    ...
    const ctx = _objectSpread({}, context);
  ...
  ['ExclusionConstraint'](node, context = {}) {
    ...
    function getExclusionGroup(nde) {
      const a = nde.exclusions.map(excl => {
        ...
        return excl[0].IndexElem.expr ? this.deparse(excl[0].IndexElem.expr, context) : null;
      });
  ...
  ['CreateFunctionStmt'](node, context = {}) {
    ...
    output.push(node.funcname.map(name => this.deparse(name, context)).join('.'));
    output.push('(');
    let parameters = [];
    ...
    if (node.options) {
      ...
      opts.forEach((option, i) => {
        ...
        switch (option) {
          ...
          case 'adminmembers':
            output.push('ADMIN');
            output.push(this.list(node.options[i].DefElem.arg, ', ', '', context));
            break;
  ...
  ['WindowDef'](node, context = {}) {
    ...
    if (node.partitionClause) {
      ...
      const clause = node.partitionClause.map(item => this.deparse(item, context));
