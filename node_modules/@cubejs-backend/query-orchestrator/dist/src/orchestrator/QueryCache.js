...
class QueryCache {
    ...
    async cachedQueryResult(queryBody, preAggregationsTablesToTempTables) {
        const replacePreAggregationTableNames = (queryAndParams) => QueryCache
            .replacePreAggregationTableNames(queryAndParams, preAggregationsTablesToTempTables);
        ...
        const cacheKeyQueries = this.cacheKeyQueriesFrom(queryBody).map(replacePreAggregationTableNames);
    ...
    getRefreshKeyRenewalThresholds(queryBody) {
    ...
    queryWithRetryAndRelease(query, values, { priority, cacheKey, external, requestId, dataSource }) {
    ...
    getQueue(dataSource = 'default') {
        if (!this.queue[dataSource]) {
            this.queue[dataSource] = QueryCache.createQueue(`SQL_QUERY_${this.redisPrefix}_${dataSource}`, () => this.driverFactory(dataSource), (client, q) => {
    ...
    loadRefreshKeys(cacheKeyQueries, expireSecs, options) {
        return cacheKeyQueries.map((q, i) => this.cacheQueryResult(Array.isArray(q) ? q[0] : q, Array.isArray(q) ? q[1] : [], q, expireSecs, {
